...
SRC_URI="http://ftp.gnu.org/gnu/tar/${MY_P}.tar.bz2
	ftp://alpha.gnu.org/gnu/tar/${MY_P}.tar.bz2
	mirror://gnu/tar/${MY_P}.tar.bz2"

LICENSE="GPL-3"
SLOT="0"
KEYWORDS="~amd64 ~ia64 ~sparc ~x86 ~x86-fbsd"
IUSE="7zip nls static userland_GNU"
...
src_unpack() {
	...
	if use 7zip ; then
		epatch "${FILESDIR}"/${P}-7zsupport.patch
		cp "${FILESDIR}"/tar.1 "${S}"
		epatch "${FILESDIR}"/${P}-7zip-man.patch
	fi
