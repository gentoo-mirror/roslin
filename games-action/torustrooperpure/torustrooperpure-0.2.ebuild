...
src_unpack(){
	...
	sed -i \
        -e 's:"\(images/[^"]*\)":"'${GAMES_DATADIR}'/'${PN}'/\1":g' -i ttp0_2/src/abagames/util/sdl/texture.d \
        -e 's:"\(sounds/[^"]*\)":"'${GAMES_DATADIR}'/'${PN}'/\1":g' -i ttp0_2/src/abagames/util/sdl/sound.d \
        -e 's:"\(barrage[^"]*\)":"'${GAMES_DATADIR}'/'${PN}'/\1":g' -i ttp0_2/src/abagames/tt/barrage.d \
        -e 's:"\(tt.prf[^"]*\)":"'${GAMES_STATEDIR}'/'${PN}'/\1":g' -i ttp0_2/src/abagames/tt/prefmanager.d \
		|| die "sed failed"
}
