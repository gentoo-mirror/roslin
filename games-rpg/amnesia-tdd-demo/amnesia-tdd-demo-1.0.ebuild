...
EOF
	...
	# Most files need to be writable or there will be a segmentation fault/random errors/strange artifacts. Frictional Games will release a patch for this.
	cd "${D}/${GAMEDIR}" || die "cd \"${D}/${GAMEDIR}\" failed"
	find . -type f \( -name *.anm \
		       -o -name *.dae_anim \
		       -o -name *.dds \
		       -o -name *.jpg \
		       -o -name *.map_cache \
		       ...
		       -o -name *.png \
		       -o -name *.tga \
		       \) -exec chmod 660 {} \; || die "chmod failed"
}
...
pkg_postinst() {
	...
	ewarn ""
	ewarn "Please check \"http://www.frictionalgames.com/forum/thread-3760.html\""
