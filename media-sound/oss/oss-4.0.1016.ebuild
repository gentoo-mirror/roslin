...
# New in this version (2008-02-06)
#	* Fixed ossxmix segmentation fault (based on Arch Linux OSS PKGBUILD:
#	 http://aur.archlinux.org/packages/oss-linux-free/oss-linux-free/PKGBUILD
...
http://voimakentta.vpalvelin.com/sources/stable/gpl/${MY_P}.tar.bz2 \
http://developer.opensound.com/sources/stable/gpl/${MY_P}.tar.bz2 "
...
src_compile() {
	...
	# As the Makefile works not well with ossxmix and seperate building works, we build it.
	cd ${WORKDIR}/build/cmd/ossxmix
	cc -oossxmix -I$WORKDIR/build/include `pkg-config --libs --cflags gtk+-2.0` *.c $CFLAGS || die "compiling ossxmix failed"
	cp ossxmix $WORKDIR/build/prototype/usr/bin || die "command `cp ossxmix ...` failed"	
}
