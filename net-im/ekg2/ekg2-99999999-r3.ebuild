...
RDEPEND="
	...
	sqlite? ( !sqlite3? ( dev-db/sqlite:0 ) )"
...
pkg_setup() {
	if ! use gtk && ! use ncurses && ! use readline && ! use remote && ! use web; then
		...
		ewarn 'enabling one of following USEflags:'
...
make_deps() {
	...
	while [ -n "$1" ]; do
		...
		while true; do
			# get next one-of, make ure spls gets empty if last
			spll=${spls%%|*}
			spls=${spls:$(( ${#spll} + 1 ))}
...
src_configure() {
	...
	scons PLUGINS=$(build_plugin_list) $(build_addopts_list) \
		...
		${MAKEOPTS} conf || die "scons failed"
